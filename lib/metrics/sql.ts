 ...
 *
 * «Штук» — целое число, а не деньги: не через `Money`, округления не просит.
 *
 * **Порядок строк — кусок S12, задача 1.** Прежде он был один и вшитый: по чистой выручке, по
 * убыванию. Подпись над таблицей при этом говорит про прибыль, и по таблице не прочитывалась.
 * Теперь порядок приходит доводом, умолчание — прибыль по убыванию, и шесть допустимых значений
 * перечислены здесь списком. Список — единственное место, где порядок превращается в текст
 * запроса: подстановка произвольной строки в `order by` невозможна по устройству, а не по
 * бдительности вызывающего.
 *
 * **Второй ключ — артикул.** Без него порядок строк с равным значением зависит от случая, и
 * перепись экрана со снимками краснела бы через раз. На мартовских числах ни одна пара значений
 * не совпадает, поэтому видимый порядок от второго ключа не меняется.
 *
 * **Счёт подставленной себестоимости — кусок S12, задача 2.** У строки продажи без цены поставщика
 * себестоимость считается запасными процентами от чистой выручки, и маржа такой строки выходит
 * ровно той же у всех — она выглядит измеренной. Две колонки говорят, скольким продажам артикула
 * цены не нашлось и сколько их всего. Считаются они **по строкам продаж**, а не по артикулам, по
 * той же причине, по которой так считает вид неполноты «строки продаж без цены поставщика»: у
 * товара бывает цена до середины месяца и не бывает после, и «подставлено целиком» о таком товаре
 * было бы неправдой. Числа эти на экран не идут — по ним говорится словом «вся» или «часть».
 */
const ТОВАРЫ_БЕЗ_ПОРЯДКА = `${MONEY_CTES}
select sku,
       ...
       count(*) filter (where price is null)::int    as rows_substituted,
...
/**
 * Шесть порядков таблицы товаров — ровно те, что называет адрес страницы.
 *
 * Ключ — значение из адреса, значение — хвост запроса. Ничего, кроме этих шести строк, в
 * `order by` попасть не может.
 */
export const ПОРЯДКИ_ТОВАРОВ = {
  'profit-desc': 'order by sum(net) - sum(cogs) desc, sku',
  'profit-asc': 'order by sum(net) - sum(cogs) asc, sku',
  'net-desc': 'order by sum(net) desc, sku',
  'net-asc': 'order by sum(net) asc, sku',
  'cogs-desc': 'order by sum(cogs) desc, sku',
  'cogs-asc': 'order by sum(cogs) asc, sku',
} as const

export type ПорядокТоваров = keyof typeof ПОРЯДКИ_ТОВАРОВ

/** Порядок, который показывается, когда в адресе о нём ничего не сказано. */
export const ПОРЯДОК_ТОВАРОВ_ПО_УМОЛЧАНИЮ: ПорядокТоваров = 'profit-desc'

/** Запрос таблицы товаров в названном порядке. */
export function запросТоваров(порядок: ПорядокТоваров): string {
  return `${ТОВАРЫ_БЕЗ_ПОРЯДКА} ${ПОРЯДКИ_ТОВАРОВ[порядок]}\n`
}

/**
 * Запрос в порядке по умолчанию. Его же читает `MONTH_ITEMS_EXTRA`: новым колонкам порядок строк
 * безразличен — они считают по своему ключу, — а два текста запроса развели бы одно правило счёта
 * на два места.
 */
export const MONTH_ITEMS = запросТоваров(ПОРЯДОК_ТОВАРОВ_ПО_УМОЛЧАНИЮ)
 ...
 * так край ступени «возвраты» сходится с чистой выручкой.
 *
 * **Чего это НЕ даёт, поправлено в круге проверки кода 3.** Нижний край ступени «постоянные» с
 * прибылью не сходится: края ступеней 5–8 накапливаются от чистой выручки, и на них садится то
 * самое расхождение округления, ради которого заведена строка под разбором. На марте край
 * «постоянных» — 1 738,54 €, а ступень прибыли — 1 738,53 €; это утверждает живая проверка марта.
 * Прежде здесь было написано, что края сходятся, — неверно на тех самых данных, на которых кусок
 * сдаётся.
 *
