 ...
 * следующим, и это названо в README как принятая цена, а не закрыто. Меньше — рискуем отобрать замок у работающего обновления;
...
export async function замокВБазе(): Promise<Замок> {
  ...
  return {
    взят: жетон !== null,
    отпустить: async () => {
      // Соединение закрывается в любом исходе: отказ самого отпускания не должен ни оставлять
      // соединение открытым, ни подменять собой настоящий исход обновления. Найдено проверкой
      // кода.
      try {
