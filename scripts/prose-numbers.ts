 ...
 *   · Число проверок и число сломов берутся **из файла прогона в ветке** — из вставленных в него
 *     сводки `npm test` и выводов прогонов. Что вставлено именно то, что вернули команды, она не
 *     знает: она сверяет отчёт и тело заявки с этим файлом, а не с самим прогоном. Неверная вставка
 *     пройдёт зелёной — но она лежит в коммите, который читает рецензент.
...
import { сверитьГолову } from './head-match.ts'
...
if (сейчас !== головаПрогона[1]) {
  ...
  const расхождение = сверитьГолову(головаПрогона[1], сейчас, после.stdout, { прогон: ПРОГОН, отчёт: ОТЧЁТ })
  if (расхождение !== null) отказ(расхождение)
}
...
числа.push({
  ...
  source: `${ПРОГОН}, строки «Всего сломов»`,
  places: [
    { file: ОТЧЁТ, anchor: '| Приборы сломов, ' },
    { file: ЗАЯВКА, anchor: 'сломов, у всех «покраснела своя»; списков — ' },
  ],
})
числа.push({
  name: 'сломов всего',
  value: String(сумма(/^Всего сломов: (\d+)\./gm)),
  source: `${ПРОГОН}, сумма «Всего сломов»`,
  places: [
    { file: ОТЧЁТ, anchor: `| Приборы сломов, ${списков} списков | ` },
    { file: ЗАЯВКА, anchor: 'Итог прогона: ' },
    { file: ПРОГОН, anchor: '| **итого** | **' },
  ],
})
числа.push({
  name: 'механизмов проверено',
  value: String(сумма(/Проверено механизмов: (\d+)\./g)),
  source: `${ПРОГОН}, сумма «Проверено механизмов»`,
  places: [{ file: ОТЧЁТ, anchor: `| Приборы сломов, ${списков} списков | ${сумма(/^Всего сломов: (\d+)\./gm)} сломов, ` }],
})
числа.push({ name: 'проверок в наборе', value: тесты[2], source: `${ПРОГОН}, сводка npm test`, places: [{ file: ОТЧЁТ, anchor: '| Набор проверок | ' }] })
числа.push({ name: 'файлов проверок', value: файлы[2], source: `${ПРОГОН}, сводка npm test`, places: [{ file: ОТЧЁТ, anchor: `${тесты[2]} проверок в ` }] })
