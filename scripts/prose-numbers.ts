 ...
 *
 * Чего команда не делает. Она сверяет только зарегистрированные числа. Число, которого нет в списке
 * ниже, она не видит; такое число обязано стоять рядом со строкой, откуда оно взято (наш замер в
 * браузере, счёт по сверенным итогам, решение владельца). Разбор таблиц — не глубже, чем нужно
 * списку: строки таблицы Markdown, разделённые `|`, без экранирования внутри ячеек.
 *
...
function numstat(...пути: string[]): Array<{ added: number; removed: number; path: string }> {
  const r = spawnSync('git', ['diff', '--numstat', 'origin/main', '--', ...пути], { encoding: 'utf8' })
