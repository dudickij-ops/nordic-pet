...
/**
 * Разрыв времени — **отказ прибора, а не строка в отчёте**. Прогон, переживший сон машины,
 * недействителен целиком: соединение с базой сна не переживает, и десяток сломов подряд получает
 * «ломает больше, чем объявлено» из-за красных проверок, которым нужна база. Поймать это можно
 * только по времени, и прибор ловит сам.
 *
...
function разрывВремени(секунды: number): string | null {
...
try {
  ...
  for (const one of chosen) {
    ...
    const разрыв = разрывВремени(секунды)
    if (разрыв !== null) {
      restoreAll()
