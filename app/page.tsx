...
export default async function HomePage({
  ...
  try {
    ...
    //
    // Перехват сужен (S6, входящая задача из S5): ловится **только** кривая форма месяца из
    // адреса. Прежде сюда попадал любой отказ отчёта — недоступная база, неназванная среда, —
    // и посетитель видел текст ошибки на странице с кодом ответа «всё хорошо», без кнопки
    // обновления и без ссылок на месяцы. Различаем по самому доводу, а не по тексту отказа:
    // текст — чужое имущество слоя метрик, он вправе меняться, а форма месяца известна здесь.
