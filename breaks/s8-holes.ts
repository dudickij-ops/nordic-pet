 ...
 *
 * Девятнадцать строк: семь на замок от подбора пароля, три на выход, три на свой вид у отказа
...
export const BREAKS: Break[] = [
  ...
  {
    id: 'window-never-releases',
    claim: 'не отпускать запирание по истечении окна',
    mustRedden: 'по истечении окна вход снова работает',
    alsoRedden: [
      {
        name: 'на пороге заперто и названо, сколько ждать',
        why: 'она сверяет, сколько именно ждать, а постоянный ответ этого числа не считает',
      },
    ],
    file: 'lib/auth/attempts.ts',
    find:
