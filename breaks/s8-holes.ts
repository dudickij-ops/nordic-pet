 ...
 *
 * Тридцать пять строк: четырнадцать на замок от подбора пароля, три на выход, пять на свой
 * вид у отказа отчёта, две на отметку свежести, восемь на замок от двух одновременных
 * обновлений, две на то, что человек вправду видит на экране, и одна на то, что новая схема
 * не выставлена наружу через API. Разбивка сходится с итогом: 14+3+5+2+8+2+1. Ссылаемся по имени, а не по номеру: вставка
...
export const BREAKS: Break[] = [
  ...
  {
    /**
     * Механизм, оставшийся было без слома: перехват вокруг обнуления счёта после удачного
     * входа. Прежний слом этой строки переехал на соседнее утверждение, и она осталась без
     * сторожа вовсе. Найдено проверкой кода.
     */
    id: 'success-turns-into-refusal',
    claim: 'превращать сбой учёта после удачного входа в отказ',
