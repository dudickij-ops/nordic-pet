 ...
 *
 * Двадцать шесть строк: двадцать одна из таблицы контракта плюс пять, которых в контракте
 ...
 * разрешающих веток. `matcher-widened` и `matcher-drops-app-path` пришли из проверки кода: у
 * образца путей первого слоя не было ни проверки, ни слома, и опечатка в нём выводила бы путь
 * из-под закрытия молча, не покраснев ничем. Их два, потому что проверка образца сторожит два
 * разных обещания: первый расширяет выведенное и краснит обращение за данными маршрута, второй
 * выводит из образца заведённый путь приложения — тот самый «завтрашний путь по забывчивости»,
 * ради которого весь кусок строился закрытым по умолчанию.
 *
