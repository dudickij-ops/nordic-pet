 ...
 *
 * Двадцать четыре строки: двадцать одна из таблицы контракта плюс три, которых в контракте
 * нет и которые появились от решений плана. Строка 7 отделяет первый слой закрытия от
 * второго — без неё «снять сторожа со страницы» и «сломать общий слой» доказывались бы одной
 * и той же проверкой. Строка 18 сторожит направление отказа при неназванном секрете: «нечем
 * проверить подпись» обязано значить «не пускаем», а не «пускаем». Строка 24 сторожит само
 * правило владельца — в стороже нет разрешающих веток.
 *
