...
export const BREAKS: Break[] = [
  // --- разбор значений ----------------------------------------------------------------
  {
    ...
    alsoRedden: [
      { name: 'дата форма с точками читается как день-месяц-год, а не наоборот', why: 'та же форма даты проверяется на других значениях' },
      { name: 'дата крайние дни месяца и високосный год разбираются', why: 'та же форма даты проверяется на других значениях' },
    ],
    file: PARSE,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'дата третья форма даты не угадывается: 01/03/2026', why: 'непонятая дата перестала быть отказом во всех её случаях и в проверках текста отказа' },
      { name: 'дата третья форма даты не угадывается: 45352', why: 'непонятая дата перестала быть отказом во всех её случаях и в проверках текста отказа' },
      { name: 'дата третья форма даты не угадывается: 2026-3-1', why: 'непонятая дата перестала быть отказом во всех её случаях и в проверках текста отказа' },
      { name: 'дата третья форма даты не угадывается: 1.3.2026', why: 'непонятая дата перестала быть отказом во всех её случаях и в проверках текста отказа' },
      { name: 'дата третья форма даты не угадывается: 2026.03.01', why: 'непонятая дата перестала быть отказом во всех её случаях и в проверках текста отказа' },
      { name: 'дата отказ называет место, значение и что делать', why: 'непонятая дата перестала быть отказом во всех её случаях и в проверках текста отказа' },
      { name: 'дата отказ несёт адрес отдельными полями, а не только текстом', why: 'непонятая дата перестала быть отказом во всех её случаях и в проверках текста отказа' },
    ],
    file: PARSE,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'дата обе формы источника разбираются в одну и ту же дату', why: 'день и месяц поменялись местами во всех значениях с точками' },
      { name: 'дата крайние дни месяца и високосный год разбираются', why: 'день и месяц поменялись местами во всех значениях с точками' },
    ],
    file: PARSE,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'число у курса шесть знаков, у процента четыре — и это разные пределы', why: 'предел знаков — один и тот же механизм для денег, курса и процента' },
    ],
    file: PARSE,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'число точка и запятая — одно и то же число', why: 'число вместо строки меняет ответ всякого разбора денег' },
      { name: 'число пробел-разделитель тысяч снимается во всех трёх написаниях', why: 'число вместо строки меняет ответ всякого разбора денег' },
      { name: 'число при двух разделителях десятичный — последний', why: 'число вместо строки меняет ответ всякого разбора денег' },
      { name: 'число целое без дробной части разбирается', why: 'число вместо строки меняет ответ всякого разбора денег' },
      { name: 'число знак минус доезжает как есть: источник не чинится', why: 'число вместо строки меняет ответ всякого разбора денег' },
      { name: 'число у курса шесть знаков, у процента четыре — и это разные пределы', why: 'число вместо строки меняет ответ всякого разбора денег' },
    ],
    file: PARSE,
...
/** Количество штук`,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'штуки не штуки — отказ: 0', why: 'тот же отказ проверяется на нуле и на отрицательном' },
      { name: 'штуки не штуки — отказ: -1', why: 'тот же отказ проверяется на нуле и на отрицательном' },
    ],
    file: PARSE,
    find: '  if (units <= 0) {',
    replace: '  if (false) {',
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'артикул три написания одного товара из источника сходятся в одно', why: 'три написания артикула сходятся именно заменой дефиса' },
    ],
    file: PARSE,
    find: "    .replace(DASHES, '-')\n",
    replace: '',
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'артикул три написания одного товара из источника сходятся в одно', why: 'без снятия пробелов не сходятся написания и не пустеет пустой артикул' },
      { name: 'артикул пустой артикул — отказ: строка без товара не ложится ни в один столбец экрана', why: 'без снятия пробелов не сходятся написания и не пустеет пустой артикул' },
    ],
    file: PARSE,
    find: "    .replace(/\\s/g, '')\n",
    replace: '',
  ...
  // --- площадка и свёртка копий -------------------------------------------------------
  {
    ...
    alsoRedden: [
      { name: 'площадка из имени файла имя, из которого площадку не вывести, — отказ: _2026-03.csv', why: 'тот же отказ проверяется на четырёх именах и на своём тексте' },
      { name: 'площадка из имени файла имя, из которого площадку не вывести, — отказ:    _x.csv', why: 'тот же отказ проверяется на четырёх именах и на своём тексте' },
      { name: 'площадка из имени файла имя, из которого площадку не вывести, — отказ: .csv', why: 'тот же отказ проверяется на четырёх именах и на своём тексте' },
      { name: 'площадка из имени файла отказ называет файл и говорит, как называть файлы', why: 'тот же отказ проверяется на четырёх именах и на своём тексте' },
    ],
    file: ADS,
    find: `  if (platform === '') {
    throw new Error(
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'свёртка копий выгрузки при равной длине имён переживает первое по порядку кодовых знаков', why: 'выбор пережившего — один механизм на три проверки' },
      { name: 'свёртка копий выгрузки сравнение не языковое: порядок не зависит от настроек машины', why: 'выбор пережившего — один механизм на три проверки' },
    ],
    file: ADS,
  ...
  // --- запись фактов ------------------------------------------------------------------
  {
    ...
    alsoRedden: [
      { name: 'fact.\'fx\': защиты есть у каждой функции записи неизменившаяся строка не переписывается: адрес версии не сдвинулся', why: 'та же защита проверяется у курса дважды: в своей проверке и в общей по семи таблицам' },
    ],
    file: MIGRATION,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'fact.\'opex\': защиты есть у каждой функции записи нулевой снимок при непустом сырье — отказ, называющий таблицу', why: 'та же защита расходов проверяется и в общей проверке по семи таблицам' },
    ],
    file: MIGRATION,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'деловые ключи две пустые даты курса тоже считаются одним ключом', why: 'без ключа не проверить и то, что две пустые даты считаются одним ключом' },
    ],
    file: MIGRATION,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'деловые ключи два курса на одну дату — отказ', why: 'неотложенный ключ рвётся раньше, чем проверки успевают дойти до своих утверждений' },
      { name: 'деловые ключи две пустые даты курса тоже считаются одним ключом', why: 'неотложенный ключ рвётся раньше, чем проверки успевают дойти до своих утверждений' },
    ],
    file: MIGRATION,
    find: '  unique nulls not distinct (date) deferrable initially deferred;',
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'процент комиссии хранится пунктами нулевая и отрицательная ставка — наш отказ: -1', why: 'тот же отказ проверяется на нуле и на отрицательном' },
    ],
    file: MIGRATION,
...

  {
    ...
    alsoRedden: [
      { name: 'нулевой снимок отказ на нулевом снимке говорит человеку, что случилось', why: 'та же защита заказов проверяется и в общей проверке по семи таблицам' },
    ],
    file: MIGRATION,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'разбор посева целиком', why: 'сумма без валюты запрещена ограничением, сборка отказывается целиком' },
      { name: 'повторный прогон и пересчёт изменившегося', why: 'то же: сборка не доходит до записи' },
      { name: 'противоречия на деловой ключ', why: 'то же' },
      { name: 'столкновение по площадке', why: 'то же' },
      { name: 'ссылки, без которых правила счёта врут молча', why: 'то же' },
      { name: 'строки-близнецы', why: 'то же' },
      { name: 'как сборка обращается с базой', why: 'то же' },
      { name: 'снимок сырья', why: 'то же' },
      { name: 'боевой путь', why: 'то же' },
      { name: 'отказы разбора', why: 'то же' },
    ],
    file: BUILD,
    find: `        currency: EUR,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'разбор посева целиком копия выгрузки свёрнута и названа в отчёте', why: 'без площадки не собирается и список площадок в отчёте' },
    ],
    file: BUILD,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'отказы базы, которые человек может исправить', why: 'сборка отказывается до записи, и подставной отказ базы не успевает случиться' },
      { name: 'разбор посева целиком', why: 'обе копии сталкиваются по деловому ключу рекламы, и сборка отказывается целиком' },
      { name: 'повторный прогон и пересчёт изменившегося', why: 'то же' },
      { name: 'строки-близнецы', why: 'то же' },
      { name: 'как сборка обращается с базой', why: 'то же' },
      { name: 'снимок сырья', why: 'то же' },
      { name: 'боевой путь', why: 'то же' },
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'противоречия на деловой ключ \'две цены на артикул и дату\' — отказ, называющий ключ', why: 'непривёденный артикул ломает и общий случай двух цен' },
    ],
    file: BUILD,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'отказы базы, которые человек может исправить', why: 'то же: выдуманное противоречие останавливает сборку раньше записи' },
      { name: 'разбор посева целиком', why: 'выдуманное противоречие останавливает всякую сборку' },
      { name: 'повторный прогон и пересчёт изменившегося', why: 'то же' },
      { name: 'строки-близнецы', why: 'то же' },
      { name: 'как сборка обращается с базой', why: 'то же' },
      { name: 'снимок сырья', why: 'то же' },
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'строки-близнецы задвоенный возврат назван так же', why: 'близнецы считаются одним механизмом на заказы и возвраты' },
      { name: 'строки-близнецы строка, отличающаяся хоть одной колонкой, близнецом не считается', why: 'близнецы считаются одним механизмом на заказы и возвраты' },
      { name: 'строки-близнецы ноль близнецов сообщается числом, а не пустым местом', why: 'близнецы считаются одним механизмом на заказы и возвраты' },
    ],
    file: BUILD,
    find: `    const twins = [
  ...
  },
  {
    ...
    mustRedden: 'задвоенная до последней колонки строка заказа названа числом и адресом',
    alsoRedden: [
      { name: 'строки-близнецы задвоенный возврат назван так же', why: 'адрес в ключе делает близнецами только совпавшие адреса, то есть никого' },
    ],
    file: BUILD,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'разбор посева целиком', why: 'чтение до открытия транзакции роняет саму сборку' },
      { name: 'повторный прогон и пересчёт изменившегося', why: 'то же' },
      { name: 'отказы разбора', why: 'то же' },
      { name: 'противоречия на деловой ключ', why: 'то же' },
      { name: 'столкновение по площадке', why: 'то же' },
      ...
      { name: 'снимок сырья', why: 'то же' },
      { name: 'боевой путь', why: 'то же' },
      { name: 'отказы базы, которые человек может исправить', why: 'то же' },
    ],
    file: BUILD,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'исчезнувшая сырая строка уносит свой факт', why: 'выпавшая строка не попадает в факты и без удаления из сырья' },
      { name: 'отказ на четвёртой из семи записей не оставляет первых трёх', why: 'заказов в снимке меньше, и обрыв приходится на другую запись' },
      { name: 'разбор посева целиком', why: 'выпавшая строка заказа меняет числа во всех проверках, которые их считают' },
      { name: 'строки-близнецы', why: 'то же' },
      { name: 'противоречия на деловой ключ', why: 'то же' },
      { name: 'ссылки, без которых правила счёта врут молча', why: 'то же' },
      { name: 'боевой путь', why: 'то же' },
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'снимок сырья сборка работает на уровне повторяемого чтения, а не на умолчании', why: 'частями записанные факты видны и по остатку после отказа' },
    ],
    file: BUILD,
...
select raw.replace_entire_ads_folder($json$[`,
...

  {
    ...
    alsoRedden: [
      { name: 'отказы базы, которые человек может исправить отказ базы \'40P01\' переведён на человеческий язык', why: 'перевод отказа — один механизм на оба кода состояния' },
    ],
    file: 'lib/facts/build.ts',
  ...
  // --- общий набор обязательств -------------------------------------------------------
  {
    ...
    alsoRedden: [
      { name: 'список команд ни одна команда package.json не заводит сценарий в обход списка', why: 'выпавшая из списка команда перестаёт числиться и в сверке с package.json' },
    ],
  ...
  // --- команда ------------------------------------------------------------------------
  {
    ...
    alsoRedden: [
      { name: 'команда разбора запускается простым node: все её части находятся', why: 'подставленная среда снимает отказ и у второго обязательства, которое им пользуется' },
    ],
    file: BUILD,
    find: '  const target = resolveIngestTarget()',
    replace:
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'команда разбора на местной базе печатает цель первой строкой и весь отчёт', why: 'исчезнувшая строка отчёта видна и общей проверке состава отчёта' },
    ],
    file: COMMAND,
    find: `  console.log(
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'команда разбора не ходит в базу, пока среда не названа', why: 'команда не запускается вовсе, поэтому молчат все её проверки' },
      { name: 'команда разбора на местной базе печатает цель первой строкой и весь отчёт', why: 'команда не запускается вовсе, поэтому молчат все её проверки' },
      { name: 'команда разбора печатает ноль близнецов числом, а не пустым местом', why: 'команда не запускается вовсе, поэтому молчат все её проверки' },
    ],
    file: COMMAND,
  ...
  },
  {
    ...
    alsoRedden: [
      { name: 'команда разбора запускается простым node: все её части находятся', why: 'без кода возврата отказ перестаёт быть отказом и для обязательства' },
    ],
    file: COMMAND,
