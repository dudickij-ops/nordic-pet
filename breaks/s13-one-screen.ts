...
export const BREAKS: Break[] = [
  {
    ...
    alsoRedden: [
      ...
      // Задача 17, правка по итоговой проверке (И3): замечено рецензентом чтением, подтверждено сухой примеркой.
      ...
      // Задача 17: проверка М6 дописана после слома; пробный запуск прибора показал её сверх объявленного.
  ...
  },
  {
    ...
    // правки его хвоста и объявлена заодно. Перепись, объявленная «на случай», снята: сухая примерка её в
    // красном не показала.
  ...
  },
  {
    ...
    // Задача 17, правка по итоговой проверке (И4): рецензент назвал чтением две необъявленные проверки, сухая
    // примерка показала пять — все читают полосу целиком и видят четвёртую строку.
    alsoRedden: [
