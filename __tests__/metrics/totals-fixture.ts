 ...
 * задачи 3 (себестоимость): подпись зафиксирована заданием и не меняется. Вынесена сюда
 * из `revenue.test.ts`, чтобы `cogs.test.ts` пользовался тем же способом класть факты, а
 * не завёл второй, который однажды разойдётся с первым молча.
 *
 * Сырьё кладётся теми же функциями снимка, что и живая загрузка (S1) и живая сборка (S4):
 * второй способ положить строку разошёлся бы с первым молча. Раз слой метрик читает
 * только `fact.*`, а у каждой строки фактов внешний ключ на свою сырую — в сырую таблицу
 * кладётся строка того же адреса, прежде чем класть факт. Само содержимое сырой строки
 * счёту не важно: счёт его не читает, — важен только адрес.
 *
 ...
 *
 * Vitest изолирует модули по тестовому файлу, поэтому `pool` и хуки `beforeAll`/`afterAll`
 * ниже создаются заново в каждом файле, который импортирует эту подставку, — общий модуль
 * не делит одно соединение на несколько наборов проверок.
 */
 ...
 */
export async function totalsOn(
  orders: OrderRow[],
  refunds: RefundRow[],
  month: string,
  costs: CostRow[] = [],
