...
describe('месячный отчёт', () => {
  ...
  test('доля честности считается от чистой выручки', async () => {
    ...
    expect(о.honesty.sharePct).toBe('90.0')
  ...
  test('товары отсортированы по чистой выручке убыванием, штуки — за вычетом возвращённых', async () => {
    const о = await reportOn({
      orders: [
        { order: 'A-1', sku: 'NP-001', date: '2026-03-02', units: 3, gross: '900.00', discount: '0.00', gateway: 'card' },
        { order: 'A-2', sku: 'NP-012', date: '2026-03-02', units: 1, gross: '100.00', discount: '0.00', gateway: 'card' },
      ],
      refunds: [{ order: 'A-1', sku: 'NP-001', date: '2026-03-09', units: 1, amount: '300.00' }],
      costs: [{ sku: 'NP-001', cost: '10.00', from: '2026-01-01' }],
      extras: {},
    }, '2026-03')
    expect(о.items.map((i) => i.sku)).toEqual(['NP-001', 'NP-012'])
