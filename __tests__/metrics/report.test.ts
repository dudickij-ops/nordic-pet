...
describe('месячный отчёт', () => {
  ...
  test('месяц без выгрузки рекламы: сумма рекламы в отчёте пуста, а не ноль', async () => {
    // Круг проверки кода 3. Отсутствие выгрузки — «данных нет», а не «реклама стоила ноль».
    // Соседние статьи рядом остаются числами: пусто здесь — про рекламу, а не про весь месяц.
    const о = await reportOn({ ...РАСКЛАДКА, extras: { ...РАСКЛАДКА.extras, ads: [], fx: [] } }, '2026-03')
    expect(о.costs.ads).toBeNull()
    expect(о.costs.fees).not.toBeNull()
    expect(о.costs.cogs).not.toBeNull()
  })
  ...
  /**
   * **Утверждение изменено куском S12, и это правка принятой проверки прежнего куска.**
   *
   * Прежде оно звучало «товары отсортированы по чистой выручке убыванием» — порядок по умолчанию
   * был такой. Кусок S12 сделал умолчанием прибыль по убыванию, и прежнее имя стало говорить о
   * том, чего нет. Оставить его значило бы держать в наборе проверку с ложным именем.
   *
   * **Раскладка тоже переписана, и по находке рецензента.** Прежние две строки не различали
   * порядков вовсе: у товара с большей выручкой была и большая прибыль, и проверка проходила при
   * любом из двух. Теперь строк три, и все три порядка — по выручке, по прибыли и по артикулу —
   * дают три разные последовательности:
   *
   *   NP-012 — выручка 600, себестоимость 580, прибыль  20
   *   NP-005 — выручка 300, себестоимость 100, прибыль 200
   *   NP-001 — выручка 100, себестоимость  40, прибыль  60 (цены нет: запасные 40 %)
   *
   * по прибыли: NP-005, NP-001, NP-012; по выручке: NP-012, NP-005, NP-001; по артикулу:
   * NP-001, NP-005, NP-012.
   */
  test('товары отсортированы по прибыли убыванием, штуки — за вычетом возвращённых', async () => {
    const о = await reportOn({
      orders: [
        ...
        { order: 'A-3', sku: 'NP-005', date: '2026-03-02', units: 1, gross: '300.00', discount: '0.00', gateway: 'card' },
      ],
      refunds: [{ order: 'A-1', sku: 'NP-012', date: '2026-03-09', units: 1, amount: '300.00' }],
      costs: [
        { sku: 'NP-012', cost: '290.00', from: '2026-01-01' },
        { sku: 'NP-005', cost: '100.00', from: '2026-01-01' },
      ],
      extras: {},
    }, '2026-03')
    expect(о.items.map((i) => i.sku)).toEqual(['NP-005', 'NP-001', 'NP-012'])
    expect(о.items[0]).toMatchObject({ units: '1', net: '300.00', cogs: '100.00', profit: '200.00' })
    expect(о.items[1]).toMatchObject({ units: '1', net: '100.00', cogs: '40.00', profit: '60.00' })
    expect(о.items[2]).toMatchObject({ units: '2', net: '600.00', cogs: '580.00', profit: '20.00' })
  })
  ...
  test('месяц не выбран — сумма рекламы тоже «нет данных», а не ноль', async () => {
    // Круг проверки кода 4. Изменение показанного, принятое владельцем 14 сентября 2026 года и
    // записанное в договоре куска: прежде здесь печаталось «0,00 €». Месяца нет — значит нет и
    // выгрузки рекламы за него, и ноль здесь сказал бы, что реклама стоила ноль евро.
    // Соседние статьи остаются нулями: у них своё правило, договор S5, и его кусок не трогает.
    const о = await reportOn({ orders: [], costs: [], extras: {} })
    expect(о.costs.ads).toBeNull()
    expect(о.costs.fees).toBe('0.00')
    expect(о.costs.fixed).toBe('0.00')
  })
 ...
 * прежняя раскладка не создавала нужного случая. Ни одна строка выше не тронута.
 */
describe('задача 10 — случаи, которых не было ни в одной прежней раскладке', () => {
  test('счётчик строк без цены поставщика считает строки, а не разные артикулы', async () => {
    // Одна и та же пара «заказ + артикул» повторена дважды под РАЗНЫМИ заказами одного
    // артикула. Счётчик по строкам обязан дать 2; счётчик по различным артикулам дал бы 1.
    const о = await reportOn({
      orders: [
        { order: 'A-1', sku: 'NP-999', date: '2026-03-02', units: 1, gross: '10.00', discount: '0.00', gateway: 'card' },
        { order: 'A-2', sku: 'NP-999', date: '2026-03-03', units: 1, gross: '10.00', discount: '0.00', gateway: 'card' },
      ],
      costs: [],
      extras: {},
    }, '2026-03')
    const дыра = о.gaps.find((g) => g.kind === 'строки продаж без цены поставщика')!
    expect(дыра.count).toBe(2)   // не 1: строк без цены две, хоть артикул один
    expect(дыра.at).toEqual(['NP-999'])
  })

  test('реклама трёх дней с одинаковым неровным курсом не теряет копейку на сумме', async () => {
    // 1,00 ÷ 3,0000 = 0,3333… у каждого дня. Округли по дням — 0,33 × 3 = 0,99. Сложи точные
    // остатки и округли один раз — почти ровно 1,00, и это разница ровно в копейку.
    const о = await reportOn({
      orders: [],
      costs: [],
      extras: {
        ads: [
          { file: 'meta.csv', row: 1, date: '2026-03-01', campaign: 'a', platform: 'meta', spend: '1.00' },
          { file: 'meta.csv', row: 2, date: '2026-03-02', campaign: 'a', platform: 'meta', spend: '1.00' },
          { file: 'meta.csv', row: 3, date: '2026-03-03', campaign: 'a', platform: 'meta', spend: '1.00' },
        ],
        fx: [
          { date: '2026-03-01', rate: '3.0000' },
          { date: '2026-03-02', rate: '3.0000' },
          { date: '2026-03-03', rate: '3.0000' },
        ],
      },
    }, '2026-03')
    expect(о.costs.ads).toBe('1.00')   // не 0.99
  })

  test('нулевая чистая выручка от реальных строк не роняет отчёт ошибкой деления', async () => {
    // Заказ и его полный возврат на ту же сумму: net = 100 − 0 − 100 = 0,00 — настоящий
    // numeric-ноль от живых строк, а не SQL NULL от пустого месяца (тот случай отдельный:
    // «при нулевой чистой выручке маржа — нет данных, а не ноль», выше). Путь достижим на
    // настоящих данных: месяц, где всё вернули.
    const о = await reportOn({
      orders: [{ order: 'A-1', sku: 'NP-001', date: '2026-03-02', units: 1, gross: '100.00', discount: '0.00', gateway: 'card' }],
      refunds: [{ order: 'A-1', sku: 'NP-001', date: '2026-03-09', units: 1, amount: '100.00' }],
      costs: [{ sku: 'NP-001', cost: '10.00', from: '2026-01-01' }],
      extras: {},
    }, '2026-03')
    expect(о.revenue.net).toBe('0.00')
    expect(о.bottom.marginPct).toBeNull()
    expect(о.honesty.sharePct).toBeNull()
  })
})
