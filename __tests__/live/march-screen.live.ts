...
async function проверитьМарт(): Promise<void> {
  ...
  // Кусок S13: каскад — семь ступеней от чистой выручки, оборота среди них больше нет.
  expect(отчёт.waterfall?.steps.map((с) => с.key)).toEqual([
    'net', 'cogs', 'ads', 'fees', 'margin_income', 'fixed', 'profit',
  ])

  // Три числа, которые владелец посчитал сам, мимо нашего кода: оборот — из строки итогов
  // (ступени водопада больше не несут оборот), чистая выручка и прибыль — из ступеней каскада.
  expect(отчёт.revenue.gross, 'оборот').toBe('18764.00')
  ...
  // Признаки выводов (кусок S13) — числа сверенного живого прогона задачи 7, не наш счёт:
  // docs/сверка/числа-марта-s13.md.
  ...
  // Средняя на ряде (кусок S13, задача 3) — число сверенного живого прогона задачи 7, не наш счёт.
  expect(отчёт.daily?.avgNet).toBe('557.32')
  expect(отчёт.daily?.avgBase).toBe('по 31 дню месяца')
  // В марте есть день без заказов (число вида — задача 0 куска S13, файл числа-марта-s13.md).
  expect(отчёт.daily?.hasEmptyDays).toBe(true)
  ...
  // Первая пятёрка по валовой прибыли (кусок S13, задача 4, решение владельца Э4) — порядок взят
  // из первых пяти строк списка товаров в docs/сверка/метрики-2026-03-после-s13.txt, а не из памяти.
  expect(отчёт.items.filter((и) => и.inTop).map((и) => и.sku)).toEqual([
    'NP-004', 'NP-003', 'NP-009', 'NP-012', 'NP-002',
  ])
