...
test('новые колонки и строка над таблицей рисуются только при своих полях', () => {
  ...
  expect(прежняя).not.toContain('Доля в валовой прибыли товаров')
  expect(прежняя).not.toContain('items-summary')
  expect(блок(сОтчётом({}))).toContain('<th>Доля в валовой прибыли товаров</th>')
})

test('заголовок доли называет базу — прибыль товаров, а заголовок маржи — чистую выручку', () => {
  const разметка = блок(сОтчётом({}))
  expect(разметка).toContain('<th>Доля в валовой прибыли товаров</th>')
...
test('строка над таблицей называет базу долей и её сумму, и прибыль месяца рядом', () => {
  const разметка = блок(сОтчётом({}))
  expect(разметка).toContain('Валовая прибыль товаров — выручка минус себестоимость, 9\u00A0090,90\u00A0€')
  expect(разметка).toContain('это не прибыль месяца, 9\u00A0999,99\u00A0€')
  expect(разметка).toContain('80\u00A0% валовой прибыли товаров дают 2 из 2 артикулов')
})
 ...
 * не то, что браузер их применил. **Подсветки нет ни на одном снимке** — поправлено в круге проверки
 * кода 1, прежде здесь стояло обратное: в марте убыточных строк 0 из 12 (живая проверка), и в
 * раскладках снимков их тоже нет. Вид подсветки глазом не принят; контраст её замерен на выдуманной
 * строке — `docs/сверка/контраст.md`.
 */
