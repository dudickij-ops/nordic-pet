...
test('нет базы для сравнения — значения трёх показателей стоят числами, пуста только дельта', () => {
  const разметка = блок(сПолосой(БЕЗ_БАЗЫ))
  const значения = [...разметка.matchAll(/<span class="kpi-value">([^<]*)<\/span>/g)].map((м) => м[1])
  expect(значения).toEqual(['2\u00A0323,23 €', '34,5 %', '2\u00A0424,24 €'])
  expect(строкиДельт(сПолосой(БЕЗ_БАЗЫ))).toEqual(Array(3).fill('нет базы: в 2026-02 заказов нет'))
})
...
test('месяца для сравнения нет вовсе — «нет базы для сравнения» без месяца', () => {
  expect(строкиДельт(сПолосой({ ...БЕЗ_БАЗЫ, prevMonth: null }))).toEqual(Array(3).fill('нет базы для сравнения'))
})
...
test('подписи называют базу отношений, дельта маржи — в процентных пунктах', () => {
  ...
  expect(подписи).toEqual(['Прибыль', 'Маржа от чистой выручки', 'Чистая выручка'])
...
test('база есть, а дельты нет — «нет данных» у этой карточки, остальные с числами', () => {
  ...
  expect(строкиДельт(сПолосой(полоса))[2]).toBe('к 2026-02: нет данных')
})
