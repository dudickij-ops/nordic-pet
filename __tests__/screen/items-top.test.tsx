 ...
 *
 * Здесь признак стоит вразброс, а артикулы не упорядочены ни по возрастанию, ни по убыванию; прибыль у
 * всех строк одна (строка раскладки переписи), так что и порядок по прибыли ничего не решает. Первые
 * пять по индексу — NP-305, NP-102, NP-410, NP-201, NP-503, а пятёрка по признаку — NP-102, NP-201,
 * NP-503, NP-007, NP-333.
 */
const ВРАЗБРОС: Array<[string, boolean]> = [
  ['NP-305', false], ['NP-102', true], ['NP-410', false], ['NP-201', true],
  ['NP-503', true], ['NP-120', false], ['NP-007', true], ['NP-333', true],
]
const вразброс = () => ВРАЗБРОС.map(([sku, inTop]) => ({ ...ПЕРЕПИСЬ.items[0], sku, inTop }))
