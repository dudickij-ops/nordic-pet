 ...
 * ячейках, и столбика у неё нет, а прочие восемь ступеней печатаются как обычно. Прежде здесь стояло
...
test('нет строк рекламы — у ступени «Реклама» слова и в сумме, и в доле, а столбика нет', () => {
  const разметка = сВодопадом({
    ...ПЕРЕПИСЬ.waterfall!,
    steps: ПЕРЕПИСЬ.waterfall!.steps.map((с) => (с.key === 'ads' ? { ...с, amount: null, sharePct: null, largest: false } : с)),
  })
