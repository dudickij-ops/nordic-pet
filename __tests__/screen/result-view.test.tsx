...
/**
 * Задача 14, часть B, решение контролёра: слово «порога нет» у окупаемости в блоке результата. Разметка берёт
 * его по признаку и не подменяет ни «нет данных» (данные есть — порога нет по делу), ни порогом числом.
 */
test('признак «порога нет» — у окупаемости слова «порога нет», а не «нет данных» и не порог числом', () => {
  const разметка = блок(сОтчётом({ findings: { ...ПЕРЕПИСЬ.findings!, adsVerdict: 'порога нет' } }))
  expect(разметка).toContain('<span class="result-verdict" data-verdict="порога нет">порога нет</span>')
  expect(разметка).not.toContain('· порог')
})
