...
const OBLIGATIONS: Obligation[] = [
  {
    name: 'запускается простым node: все её части находятся',
    check: (command) => {
  ...
  {
    name: 'неназванная среда — отказ до всякой работы',
    check: (command) => {
  ...
  {
    name: 'цель названа до первой работы, а не после неё',
    check: (command) =>
      withTarget(async () => {
...
describe('список обязательств', () => {
  test('обязательств ровно пять, и они те самые', () => {
    ...
    expect(OBLIGATIONS.map((one) => one.name)).toEqual([
      'запускается простым node: все её части находятся',
      'неназванная среда — отказ до всякой работы',
      'цель названа до первой работы, а не после неё',
