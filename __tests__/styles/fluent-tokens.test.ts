 ...
 *
 * Их шесть, и список закрыт: всё, что не здесь и не в шкалах, красит закрывающее утверждение.
 * Шестую добавил кусок S12 — предел ширины пометки в ячейке таблицы; счёт снят командой по файлу.
...
const ИСКЛЮЧЕНИЯ_ПРАВИЛ: Record<string, string> = {
  '11rem':
    'предел ширины пометки подставленной себестоимости в ячейке таблицы (кусок S12): без него ' +
    'пометка держала бы колонку шире себя и уезжала за правый край узкого экрана; ширины подписи ' +
    'в ячейке Fluent не называет вовсе, и число взято по снимку узких «Товаров»',
  '26rem':
