 ...
 *
 * Руками написанной таблицы для этого мало, и ровно по той причине, ради которой весь кусок
 * строился закрытым по умолчанию: завтрашний путь приложения в неё никто не впишет. Заводим
 * `app/отчёт/page.tsx`, дописываем в образец `|отчёт` — и руками написанный список зелен, а
 * путь открыт настежь, потому что слой на него больше не зовут. Соседняя проверка тут не
 * помогает: она зовёт `proxy()` напрямую и образца не касается вовсе. Поэтому пути читаются
 * с диска — той же `путиПриложения()`, — и добавка ловит именно этот случай: путь, который
 * в приложении завели, а из-под образца вывели.
 */
test('образец путей зовёт первый слой на всё, кроме сборочной выдачи', () => {
  ...
  // Та же опора, что у соседней проверки: пустой список с диска сделал бы добавку
  // бессмысленной, и проверка зеленела бы ни о чём.
  ...
  for (const путь of пути)
    expect(
      накрыт(путь),
      `путь ${путь} заведён в приложении, но выведен из-под образца: слой на него не позовут`,
    ).toBe(true)
