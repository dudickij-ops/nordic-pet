...
test('страница входа не показывает ни чисел отчёта, ни ссылок на него', () => {
  ...
  // Требуется отсутствие якоря вовсе, а не отсутствие ссылки на корень: `href="/dashboard"`
  // ведёт на отчёт ровно так же, а образец по одному написанию его пропустил бы. Страница
  // входа никуда не ссылается по устройству — ей неоткуда взять адрес, годный до входа.
  expect(html, 'ссылки на отчёт со страницы входа не ведут').not.toMatch(/<a[\s/>]/i)
})
