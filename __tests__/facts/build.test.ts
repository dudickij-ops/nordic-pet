...
describe('боевой путь', () => {
  ...
  test('боевой прогон прибирает за собой, в каком бы порядке ни шли проверки', async () => {
    ...
    const { rows } = await pool.query(
      `select coalesce(sum(n), 0)::int as total from (
         select count(*) as n from fact.orders union all select count(*) from fact.refunds
         union all select count(*) from fact.costs union all select count(*) from fact.fees
         union all select count(*) from fact.opex union all select count(*) from fact.fx
         union all select count(*) from fact.ads) as все`,
    )
    expect(rows[0].total).toBe(0)
  })
})
