...
/**
 * Эталоны для проверок сторожа кадра — свежие, из тех же состояний генератора, во временный каталог.
 *
 * Сторож кадра стоит после сверки разметки с эталоном. Против закоммиченных эталонов любой прежний
 * слом, меняющий разметку, ронял бы сверку раньше, и эти проверки краснели бы не о кадре — найдено
 * сплошным прогоном прежних списков на голове слияния: слом S9 о классе устаревших чисел. Сверку с
 * закоммиченными эталонами держат проверки генератора S10; здесь проверяется только кадр.
 *
 ...
 */
function свежиеЭталоны(): string {
  const каталог = временный('эталоны')
  if (!каталог.startsWith(tmpdir())) throw new Error(`эталоны проверки — только во временный каталог, а не в «${каталог}»`)
...
test('генератор отказывает, когда нижний край водопада или ряда ниже края кадра', () => {
  const пойман = отказ(() =>
    собратьСнимки({ куда: временный('снимки'), эталоныИз: свежиеЭталоны(), толькоПары: true, браузер: подставной(99_999) }),
  )
...
test('замер кадра без ответа — отказ замера, а не ноль', () => {
  const пойман = отказ(() =>
    собратьСнимки({ куда: временный('снимки'), эталоныИз: свежиеЭталоны(), толькоПары: true, браузер: подставной(0) }),
  )
  expect(пойман.вид).toBe('замер не удался')
})

test('край в пределах кадра — снимки собраны', () => {
  const итог = собратьСнимки({ куда: временный('снимки'), эталоныИз: свежиеЭталоны(), толькоПары: true, браузер: подставной(1200) })
