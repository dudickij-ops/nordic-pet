...
import { СОСТОЯНИЯ, величиныТемы, готовыеСтраницы, собратьСнимки, type Браузер } from '@/docs/screens/generate'
...
/**
 * Окружение проверок сторожа кадра — своя маленькая таблица стилей и свежие эталоны из тех же состояний
 * генератора, всё во временном каталоге.
 *
 * Сторож кадра стоит после разбора таблицы стилей и сверки разметки с эталоном. Против настоящей таблицы
 * стилей и закоммиченных эталонов любой прежний слом, меняющий разметку или таблицу стилей, ронял бы
 * разбор или сверку раньше, и эти проверки краснели бы не о кадре — найдено сплошным прогоном прежних
 * списков на голове слияния: слом S9 о классе устаревших чисел и слом S10 о третьем блоке величин.
 * Разбор настоящей таблицы стилей и сверку с закоммиченными эталонами держат проверки генератора S10;
 * здесь проверяется только кадр.
 *
 * Пишется только во временный каталог, и это сторожится здесь же: проверка, способная переписать
 * сданные эталоны, опаснее дефекта, который здесь чинится.
 */
function окружение(): { эталоныИз: string; стили: string } {
  const каталог = временный('окружение')
  if (!каталог.startsWith(tmpdir())) throw new Error(`окружение проверки — только во временный каталог, а не в «${каталог}»`)
  const стили = join(каталог, 'globals.css')
  const css = ':root { --bg: #ffffff; }\n@media (prefers-color-scheme: dark) {\n  :root { --bg: #000000; }\n}\n'
  writeFileSync(стили, css)
  for (const готовая of готовыеСтраницы(css, величиныТемы(css), СОСТОЯНИЯ)) {
    writeFileSync(join(каталог, готовая.имя), готовая.текст)
  }
  return { эталоныИз: каталог, стили }
}
...
test('генератор отказывает, когда нижний край водопада или ряда ниже края кадра', () => {
  const пойман = отказ(() =>
    собратьСнимки({ куда: временный('снимки'), ...окружение(), толькоПары: true, браузер: подставной(99_999) }),
  )
...
test('замер кадра без ответа — отказ замера, а не ноль', () => {
  const пойман = отказ(() =>
    собратьСнимки({ куда: временный('снимки'), ...окружение(), толькоПары: true, браузер: подставной(0) }),
  )
  expect(пойман.вид).toBe('кадр не замерен')
})

test('край в пределах кадра — снимки собраны', () => {
  const итог = собратьСнимки({ куда: временный('снимки'), ...окружение(), толькоПары: true, браузер: подставной(1200) })
